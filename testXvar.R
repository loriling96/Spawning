...
ui <- fluidPage(
  ...
  column(3,
         ...
         selectInput(inputId ="xvar", label = "Select X-axis variable", choices = c("Spawn.Date", "Lunar.age"), selected = "Spawn.Date"),
...
server <- function(input, output){
  output$spawnplot <-renderPlot({ 
      ...
      S<-SpawnDF %>%
        ...
        group_by_(input$xvar) %>% 
        count(Spawn.Type)
      print(S)
        ggplot(S, aes_string(x=input$xvar, y=n, fill=Spawn.Type)) +
